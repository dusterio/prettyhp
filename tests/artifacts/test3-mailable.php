...
class Mailable implements MailableContract
{
    ...
    public function queue(Queue $queue)
    {
        ...
        if ($queueName) {
            ...
            return $queue->connection($connection)->push(new SendQueuedMailable($this));
        }
    }
    ...
    {
        if (! empty($this->from)) $message->from($this->from[0]['address'], $this->from[0]['name']);
    ...
    {
        $this->callbacks[] = function ($message) use ($level) { $message->setPriority($level); };
    ...
    {
        if (Str::startsWith($method, 'with')) return $this->with(Str::snake(substr($method, 4)), $parameters[0]);
